				...
				if(var2.toLowerCase().startsWith("save-all")) {
					...
					if(var2.toLowerCase().startsWith("op ")) {
						...
						if(var2.toLowerCase().startsWith("ban ")) {
							...
							else if (var2.toLowerCase().startsWith("weather ")) {
								var13 = var2.substring(var2.indexOf(" ")).trim();
								var14 = null;

								if ("clear".equalsIgnoreCase(var13)) {
									this.minecraftServer.worldMngr[0].forceWeatherClear();
									this.sendNoticeToOps(var4, "Weather is set to clear");
								}
								else if ("rain".equalsIgnoreCase(var13) || "raining".equalsIgnoreCase(var13)) {
									this.minecraftServer.worldMngr[0].forceWeatherRain();
									this.sendNoticeToOps(var4, "Weather is set to raining");
								}
								else if ("thunder".equalsIgnoreCase(var13) || "thundering".equalsIgnoreCase(var13)) {
									this.minecraftServer.worldMngr[0].forceWeatherThunder();
									this.sendNoticeToOps(var4, "Weather is set to thundering");
								}
								else {
									this.sendNoticeToOps(var4, "Bad syntax!");
									this.sendNoticeToOps(var4, "Allowed command arguments: ");
									this.sendNoticeToOps(var4, "- clear");
									this.sendNoticeToOps(var4, "- rain & raining");
									this.sendNoticeToOps(var4, "- thunder & thundering");
								}
